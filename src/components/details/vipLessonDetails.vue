    ...
    <div :class="ptSalesPage == 10 ? 'layoutR-main' : '' ">
        <div class="detail-box">
            <header class="index-item-title flex-between">
                <div class="title">会籍会员上课详情 {{ptSalesPage}}</div>
                <div class="vipNav-rBox">
                    <monthSceen  @getMonthScreen="getMonthScreen"></monthSceen>
                    <el-button v-show="ptSalesPage == 5" class="btn-ptMore" @click="btnTotalMore('tabPaneState','vLessonD')">
...
<script>
    ...
    import {VipMemberClass} from '@/assets/js/api' /*引用 会员总览 接口*/
    export default {
        name: "statisDetails",
        props:{
            ptSalesPage:{
