	...
	export default {
		...
		data() {
			return {
				...
				// 私教预约按钮组
				btnRecharge:[
					{ name:"信息编辑", type:'if',  iconClass:'icon-chongzhi-',  methodsName:'menberRecharge' ,pageName:'menberRecharge'},
					{ name:"课程删除", type:'if',  iconClass:'bgcMediumBlue2 icon-jiaolian', methodsName:'btnAssign', pageName:'addNewVip'},
					{ name:"上下架设置", type:'if',  iconClass:'bgcPink icon-chongzhi', methodsName:'btnDelVip', pageName:'addNewVip'},
					{ name:"专卡一对一", type:'if',  iconClass:'bgcPink icon-chongzhi', methodsName:'btnDelVip', pageName:'addNewVip'},
					{ name:"添加私教课", type:'if',  iconClass:'bgcPink icon-chongzhi', methodsName:'btnAddLessonPT', pageName:'addNewVip'},
				],
            }
		},
		methods: {
			...
			btnMethods(methodsName,pageName) {
				...
				/*添加私教课*/
				if(methodsName == 'btnAddLessonPT'){
					console.log('btnAddLessonPT');
					this.$emit('goAddPtLesson');
